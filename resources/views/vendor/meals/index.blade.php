@extends('layouts.vendor.index')

@section('content')
<div id="content" class="loading_wrapper"></div>
@endsection
@section('modal')
<div class="modal fade" id="addMeal">
  <div class="modal-dialog">
    <div class="modal-content">

      <!-- Modal Header -->
      <div class="modal-header">
        <h5>Add Meal</h5>
        <button type="button" class="close" data-dismiss="modal">&times;</button>
      </div>

      <!-- Modal body -->
      <div class="modal-body">
        <form action="{{route('vendor.post_create_meals')}}" id="addMealForm" method="post">
            @csrf
          <div class="row">
            <div class="col-md-4">                  
                <input type="text" name="name[]" class="form-control" placeholder="Enter Meal Name...">
            </div>
            <div class="col-md-3">                  
                <input type="text" name="price[]" class="form-control" placeholder="Price ...">
            </div>
            <div class="col-md-3">                  
                <select class="form-control" name="status[]">
                    <option value="active">Active</option>
                    <option value="inactive">Inactive</option>
                </select>
            </div>
            <div class="col-md-2">
                <button class="btn btn-sm btn-success addMore" type="button"><i class="ion-ios-plus-outline"></i></button>
            </div>
          </div>  
          <div class="newField"></div>
      </div>

      <!-- Modal footer -->
      <div class="modal-footer">
        <button type="submit" class="btn btn-primary">Submit</button>
        <button type="button" class="btn btn-danger" data-dismiss="modal">Close</button>
      </div>
      </form>
    </div>
  </div>
</div>

<div class="modal fade" id="editMeal">
  <div class="modal-dialog">
    <div class="modal-content">

      <!-- Modal Header -->
      <div class="modal-header">
        <h5>Edit Meal</h5>
        <button type="button" class="close" data-dismiss="modal">&times;</button>
      </div>

      <!-- Modal body -->
      <div class="modal-body" id="editMealBody">
      </form>
    </div>
  </div>
</div>
@endsection
@section('scripts')
<script type="text/javascript">
  loadData();
  function loadData(){
    $.ajax({
        type: "get",
        url: "{{route('vendor.get_meals')}}",
        data: "",
        cache: false,
        beforeSend: function() {
            $(".loading_wrapper").addClass('loading');         
        },
        success: function (data){
          $(".loading_wrapper").removeClass('loading'); 
          $("#content").html(data);
          },
          error:function(data){
            console.log(data);
          }
      });
  }
  function deleteMeal(i){
     $.ajax({
         type:'POST',
         url: "{{route('vendor.delete_meal')}}",
         data:{"id":i}, 
         dataType:'json',         
         success:function(data){
             toastr.success(data.message);
             loadData();
         },
         error: function(data){
             console.log("error");
             console.log(data);
         }
     });    
  }
  var maxGroup = 10;
  var i=1;        
  //add more fields group
  $(".addMore").click(function(){         
      if(i < maxGroup){
          var fieldHTML =

         '<div class="row" id="f-'+i+'">'+
              '<div class="col-md-4">'+               
                  '<input type="text" name="name[]" class="form-control" placeholder="Enter meal name...">'+
              '</div>'+
              '<div class="col-md-3">'+               
                  '<input type="text" name="price[]" class="form-control" placeholder="Price">'+
              '</div>'+
              '<div class="col-md-3">'+               
                  '<select class="form-control" name="status[]">'+
                      '<option value="active">Active</option>'+
                      '<option value="inactive">Inactive</option>'+
                  '</select>'+
              '</div>'+
              '<div class="col-md-2">'+
                  '<button class="btn btn-sm btn-primary" onclick="remove('+i+')" type="button"><i class="ion-ios-trash-outline"></i></button>'+
              '</div>'+
          '</div>'; 
          
          $(".newField").append(fieldHTML);
          i++;
      }else{
          alert('Maximum '+maxGroup+' groups are allowed.');
      }
  });     
  
  function remove(a){
      $("#f-"+a).remove();
      i--;
  }
  //Add Photos
  $('#addMealForm').on('submit',(function(e) {
      e.preventDefault();
      var data=$(this).serialize();
      $.ajax({
          type:'POST',
          url: $(this).attr('action'),
          data:data,            
          success:function(data){
              if(data.errors){                        
                for (var error in data.errors) {
                   toastr.warning(data.errors[error]);                       
                }
              }else{ 
                $('#addMealForm')[0].reset();
                $("#addMeal").modal('hide');  
                toastr.success(data.message);   
                loadData();
              }
          },
          error: function(data){
              console.log("error");
              console.log(data);
          }
      });
  }));
  $(document).on('click','.edit-btn',function(){
      $("#editMeal").modal('show');
      var id=$(this).attr('data-p-id');
      $.ajax({
          type:'GET',
          url: "{{route('vendor.get_edit_meal')}}", 
          data:{"id":id},                
          dataType:'html',                          
          success:function(data){                    
              $("#editMealBody").html(data);
          },
          error: function(data){
              console.log("error");
              console.log(data);
          }
      });         
  });
  $(document).on('submit','#editMealForm',function(e){
    e.preventDefault();
    var data=$(this).serialize();
    $.ajax({
        type:'POST',
        url: $(this).attr('action'),
        data:data,         
        success:function(data){
          if(data.errors){                        
            for (var error in data.errors) {
               toastr.warning(data.errors[error]);                       
            }
          }else{ 
            $('#editMealForm')[0].reset();
            $("#editMeal").modal('hide');  
            toastr.success(data.message);   
            loadData();
          }
        },
        error: function(data){
            console.log("error");
            console.log(data);
        }
    });
  });
</script>
@endsection


