...
<div class="row">
  <div class="col-md-12 mx-auto">
      ...
      <form id="basic-details-form" method="post" action="{{route('admin.edit_package',['slug'=>$room->slug])}}" enctype="multipart/form-data">
          @csrf
...
<script type="text/javascript">
    $(document).on('click','.del-img',function(e){
        ...
        $.ajax({
          type: "post",
          url: "{{route('admin.get_delete_photos_rooms',['id'=>$room->id])}}",
          data: {"img_id":img_id,"type":"admin"},
          cache: false,
          beforeSend: function() {
    ...
    $(document).on('click','.delete_amenity',function(e){
        ...
        $.ajax({
          type: "post",
          url: "{{route('admin.delete_amenities_rooms',['id'=>$room->id])}}",
          data: {"aid":aid,"type":"admin"},
          cache: false,
          beforeSend: function() {
              //$("body").addClass('loading');         
          },
