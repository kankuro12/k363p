...
use App\Model\VendorUser\UserCoupon;
...
class BookingController extends Controller
{
    ...
    public function get_booking_process_start_step_1(Request $request,$vslug,$rslug){
        ...
        $coupon_detail=Session::get('coupon_applied');
        $coupon=Coupon::where('coupon_code',$coupon_detail)->with('coupon_setting')->first();
        $countries=Country::all();
        $user=Auth::user();
        $meals=$hotel_detail->meals;
        $pmethods=PaymentMethod::where('status','active')->get();
        $khalti=PaymentMethod::where('name','khalti')->first();
    	return view('public.booking.index',compact('hotel_detail','room','details','countries','coupon_detail','coupon','user','meals','pmethods','khalti'));
    }
    public function apply_coupon(Request $request){
        ...
        $vid=Auth::user()->vendoruser->id;
        $applied=UserCoupon::where('coupon_id',$coupon->id)->where('vendoruser_id',$vid)->count();
        if($applied==0){
            if($this->chech_room_if_coupon_is_enabled($room_id)){
                if($this->check_coupon_detail($coupon_code,$room_id)){
                    $success=1;
                    $msg='The coupon code is valid';
                    Session::push('coupon_applied',$coupon_code);
                    $vc=new UserCoupon();
                    $vc->vendoruser_id=$vid;
                    $vc->coupon_id=$coupon->save();
                    $vc->save();
                    $coupon->coupon_setting->issued_number_coupon--;
                    $coupon->coupon_setting->save();

                }else{
                    $msg="The coupon is invalid or expired.";
                    $success=0;
                }

            }else{
                $msg="The room is not capable to apply the coupon.";
                $success=0;

            }
        }else{
            $msg="The coupon limit is exceeded.";
            $success=0;            
        }
    ...
    public function chech_room_if_coupon_is_enabled($room_id){
    ...
    public function check_coupon_detail($coupon,$room_id){
        $coupon=Coupon::where('coupon_code',$coupon)->whereDate('end_time', '>=', Carbon::now())->first();
        $room=Room::find($room_id);
        if($coupon && $room->coupon_enabled==$coupon->id){
            return true;
        }else{
            return false;
        }       
    ...
    public function booking_process_start_step_2(Request $request,$provider=null,$token=null,$idx=null){
        ...
        $booking->old_price=$price;
        $booking->num_rooms=$num_rooms;
        $booking->new_price=$price-$discount_value;
        ...
        $booking->booking_id=str_random(32).time();
        if($coupon_applied){
            $booking->coupon_applied=$coupon_applied;
        ...
        $redirect_url=route('get_invoice',['id'=>$booking->id]);

        Mail::to($email)->send(new SendInvoice($booking));
        //Notification
        $room=Room::find($room_id);
        $notification['title']=$fname." ".$lname." has booked your room ".$room->name;
        $notification['detail']='';
        $notification['link']=route('vendor.show_booking',['id'=>$booking->id]);
        $when = now()->addSeconds(5);        
        $room->vendor->user->notify((new VendorNoti($notification))->delay($when));

        return response()->json(['msg'=>'successfully booked','success'=>1,'redirect_url'=>'/']);
    }
    public function get_invoice($id){
        $user=Auth::user()->vendoruser;
        $booking=Booking::where(['id'=>$id,'user_id'=>$user->id])->first();
        if(!$booking){
            return "Unautorized";
        }
        return view('public.booking.invoice',compact('booking'));
    }
    public function checkAvailabitlity(Request $request,$vslug,$rslug){
        ...
        if($request->num_rooms){
            ...
            // $b=Booking::where('room_id',$room->id)->whereBetween('check_in_time', [$check_in_time, $check_out_time])
            // ->orWhereBetween('check_out_time', [$check_in_time, $check_out_time])->sum('num_rooms');
